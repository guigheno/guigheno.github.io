...
function updateLanguageUI() {
    const textElements = document.querySelectorAll('[data-text-pt], [data-text-en]');

    textElements.forEach(element => {
        const ptText = element.getAttribute('data-text-pt');
        const enText = element.getAttribute('data-text-en');

        element.textContent =
            AppState.currentLang === 'en' && enText
                ? enText
                : ptText;
    });

    const placeholderElements = document.querySelectorAll('[data-placeholder-pt], [data-placeholder-en]');
    placeholderElements.forEach(element => {
        const ptPlaceholder = element.getAttribute('data-placeholder-pt');
        const enPlaceholder = element.getAttribute('data-placeholder-en');

        element.setAttribute(
            'placeholder',
            AppState.currentLang === 'en' && enPlaceholder
                ? enPlaceholder
                : ptPlaceholder
        );
    });
    ...
    if (langToggle) {
        ...
        if (langText) {
            langText.textContent = AppState.currentLang === 'pt' ? 'EN' : 'PT';
        }
    }
}
...
function handleFormSubmit(e) {
    ...
    const message = AppState.currentLang === 'en'
